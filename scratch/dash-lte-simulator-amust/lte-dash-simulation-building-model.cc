...
ApplicationContainer GetDASHClientApplication (string ssMPDURL, Ptr<Node> ueNode, double simTime, uint32_t remoteHostId){
 ...
 //   LogComponentEnable ("PfFfMacScheduler", LOG_LEVEL_ALL);
   ...
   if (epc)
     {
       ...
       //Store IP adresses
       std::string addr_file="output/lte/dualstripe/addresses";
   ...
   if (generateRem)
     {
       PrintGnuplottableBuildingListToFile ("output/lte/dualstripe/buildings.txt");
       PrintGnuplottableEnbListToFile ("output/lte/dualstripe/enbs.txt");
       PrintGnuplottableUeListToFile ("output/lte/dualstripe/ues.txt");

       remHelper = CreateObject<RadioEnvironmentMapHelper> ();
       remHelper->SetAttribute ("ChannelPath", StringValue ("/ChannelList/2"));
       remHelper->SetAttribute ("OutputFile", StringValue ("output/lte/dualstripe/lena-dual-stripe.rem"));
   ...
   std::string DashTraceFile = "output/lte/dualstripe/report.csv";
   ...
   anim = new AnimationInterface ("output/lte/dualstripe/animation.xml");
   anim->AddResource("");
   ...
   flowMon->SerializeToXmlFile("output/lte/dualstripe/FlowMonitor.xml", true, true);
